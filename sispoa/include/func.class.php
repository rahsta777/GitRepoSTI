...
/***==========================Todo Planes operativos==============================================**/
	...
	function insertar_ids_dir_func($campo){
		if($this->con->conectar()==true){
			//print_r($campo);
			 echo'<script type="text/javascript">
    alert("paso a registrar  Login: '.$campo[0].'  Dependencia: '.$campo[1].'")
     </script>';
			return mysql_query("INSERT INTO tb_direcc_funcnary (id_funcnary, id_direcc) VALUES ('".$campo[0]."','".$campo[1]."')");
		}
	}
	...
	function insertar_user($campos){
		if($this->con->conectar()==true){
			 echo'<script type="text/javascript">
    alert("paso a registrar  Login: '.$campos[0].'  Dependencia: '.$campos[1].'")
     </script>';
			//print_r($campos);
			//echo "INSERT INTO cliente (nombres, ciudad, sexo, telefono, fecha_nacimiento) VALUES ('".$campos[0]."', '".$campos[1]."','".$campos[2]."','".$campos[3]."','".$campos[4]."')";
	...
	/***===================================fin Todo direcciones=====================================0 **/
	function mostrar_program_poa2(){
		if($this->con->conectar()==true){
			return mysql_query("SELECT id_dir, indicador FROM tb_productos ");
		}
	}
	function mostrar_direcc_poa2($valor0){
		if($this->con->conectar()==true){
			return mysql_query("SELECT 	id_direcciones, nomb_direcciones FROM tb_Direcciones WHERE  id_direcciones = '".$valor0."' ");
		}
	}
