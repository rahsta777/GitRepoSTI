...
<head>
  ...
  <script>
function buscaprove_cita(val_cita){
        var pasaval = {
                "varcitas" : val_cita /*,
        ...
        $.ajax({
                data:  pasaval,
                url:   'localizar_prove_cita.php',
                type:  'post',
                beforeSend: function () {
                        $("#la_consulta_provecitas").html("Procesando, espere por favor...");
                },
                success:  function (response) {
                        $("#la_consulta_provecitas").html(response);
                }
        });
}
</script>
...
<div style="float:left ;">
	<div><h1 id="bcitas">Control de Citas de Compromisos por Pagar</h1></div>
				  	
                       <div id="boxcita"  >
   		                            																	              																			
									    <div  id="bcitas" >Buscar: </div>
                                        <div >
                                         <input type="text"  name="buscaprovecitas" id="buscaprovecitas" size="10" style="background:url(images/fondo_input/fondo_input.png);width:100px;height:30px ;" onkeyup="buscaprove_cita($('#buscaprovecitas').val());return false;" />
	   	                           </div>
  	      				   <div id="la_consulta_provecitas"></div>
				        </div>       
