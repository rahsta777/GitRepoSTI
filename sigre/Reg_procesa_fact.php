...
function procedur_stat_fact(idfact){
  ...
  var id_fact3 = $("#pasavalorcitas").val();
  
   if((codg_ret0 !="Ninguno") && (codg_ret1 !="Ninguno"))
      alert("Ee aplicaran las retenciones "+codg_ret0+"% "+codg_ret1+"% y Se remite correo con aviso de Pago");
       else if((codg_ret0 !="Ninguno") && (codg_ret2 !="Ninguno")) 
         alert("Ee aplicaran  las retenciones "+codg_ret0+"% "+codg_ret2+"% y Se remite correo con aviso de Pago");
          else if((codg_ret1 !="Ninguno") && (codg_ret2 !="Ninguno"))
          alert("Ee aplicaran las retenciones "+codg_ret1+"% "+codg_ret2+"% y Se remite correo con aviso de Pago");

   if((codg_ret0 !=codg_ret1) && (codg_ret0 !=codg_ret2) && (codg_ret1!=codg_ret2) ) {


        var parametros2 = {
                "fact" : id_fact3 , "codg_xret0" :codg_ret0 , " codg_xret1" : codg_ret1, "codg_xret2" : codg_ret2
        };
        $.ajax({
                data:  parametros2,
                url:   'savefactura_procsda.php',
...
<td height="20" align="center" colspan="4" ><a HREF='citas.php' onClick="javascript:window.close(self)" ><img src="images/Exit.png" title="Cerrar"  width="26" height="26"  />Cerrar</a></td>
        ...
        <tr>
	      <td colspan="9" align="center" bgcolor="#ccc" class="box1" style='color:#0033ff;font:font-family:bold' ><strong >Reg. de Factura a Procesar</strong>  </td>
	    </tr>
     ...
     if (isset($_GET["varreg_procesa"]))
        {      
        
        include("conexion/Conexion1.php");
        $link=Conectarse(); 
       
        $buscar_cita=trim($_GET["varreg_procesa"]);
        $matris = explode('*',$buscar_cita);
				$bus1=$matris[0];
        $bus2=$matris[1];
        $buscar_cita0=trim($bus1);
          //echo "id prove ".$bus1." fact ".$bus2;
       $busqueda=mysql_query("SELECT * FROM tbl_fact  WHERE  id_fact ='".$buscar_cita0."' " );
		    if($busqueda ==FALSE) {
           ...
           while($filas=mysql_fetch_array($busqueda))
           {
             ...
             if(isset($filas['id_fact'])) 
               {
                               $fact=$filas['id_fact'];
                                $id_provfact=$filas['idprove'];
                                $desc_provfact=$filas['descrip'];
                                $fech_fact_x=$filas['fecha_fact'];
                                 $montofact_x=$filas['monto_fact'];
                                  $provefatc_x=$filas['idprove'];
                                // echo "id Fact ".$fact." Monto fact; ".$montofact_x. "fecha Fact: ".$fech_fact_x;
                               
                  }
        		$busqueda2 =mysql_query("SELECT * FROM tbl_prov WHERE prov_rif='".$provefatc_x."'");
                    
            	   	 while($filas2=mysql_fetch_array($busqueda2))
                                {	
                                $i  = $i + 1;
                                 $id=$filas2['prov_rif'];
                                $nomb_prov=$filas2['razon_prov'];
                                $represent=$filas2['repres_prov'];
                                /*	
                                $fact=$filas2['id_fact'];
       ...
       /***********************convierte DE AMERICANA a cifra LATINA separador mil con punto .**************************/
               ...
               $id_pasacitas=$id."*".$fact."*".$fech_fact_x;
                printf("<td bgcolor='#ccc'><font size='3'>&nbsp;%s</td> <td ><font size='3'>&nbsp;%s</td> <td bgcolor='#ccc'><font size='3'>&nbsp;%s</td> <td><font size='3'>&nbsp;%s</td> <td bgcolor='#ccc'><font size='3'>&nbsp;%s</td> <td bgcolor='#ccc'><font size='3'>&nbsp;%s</td> ", $id, $nomb_prov, $represent, $fech_fact_x, $fact, $montofact_r1);
                /***============================================================================================================***/
 ?> 
     <td> <input type="checkbox"  name="pasavalorcitas" id="pasavalorcitas" value="<?php echo $fact  ; ?>"/>
    ...
    <?php   /***============================================================================================================***/ // }            
        ...
        ?>
         <!---------------------------- modal content -------------------------------->
          ...
          <tr>
          <td>
          <p><strong>Seleccione Retenciones a Aplicar</strong>  </p>
          </tr>
          </td>
          <hr>
           <tr>
...
<?php
  $link3 = Conectarse();
              $cursor_ret ="SELECT * from  tip_retencion";
                    $result_ret=mysql_query($cursor_ret, $link3);
                    echo "<td><select name='codg_ret0' id='codg_ret0'>";
                               while($row_ret = mysql_fetch_array($result_ret)){
                                 ?>
                               <option value= "<?= $row_ret['VALOR']; ?> "><?=$row_ret['cod_ret']." ".$row_ret['VALOR']; ?></option>
                                 <?php
                                   }
                                    ?>
                                      ...
                                      <?php
  $link31 = Conectarse();
              $cursor_ret1 ="SELECT * from  tip_retencion";
                    $result_ret1=mysql_query($cursor_ret1, $link31);
                    echo "<td><select name='codg_ret1' id='codg_ret1'>";
                               while($row_ret1 = mysql_fetch_array($result_ret1)){
                                 ?>
                               <option value= "<?= $row_ret1['VALOR']; ?> "><?=$row_ret1['cod_ret']." ".$row_ret1['VALOR']; ?></option>
                                 <?php
                                   }
                                    ?>
                                      <option selected>Ninguno </select></select>
                                      ...
                                      <?php
  $link32 = Conectarse();
              $cursor_ret2 ="SELECT * from  tip_retencion";
                    $result_ret2=mysql_query($cursor_ret2, $link3);
                    echo "<td><select name='codg_ret2' id='codg_ret2'>";
                               while($row_ret2 = mysql_fetch_array($result_ret2)){
                                 ?>
                               <option value= "<?= $row_ret2['VALOR']; ?> "><?=$row_ret2['cod_ret']." ".$row_ret2['VALOR']; ?></option>
                                 <?php
                                   }
                                    ?>
                                      <option selected>Ninguno </select></select>   
           ...
           <td><div> 
        <a href="javascript:void(0)" ><img style="cursor:pointer;width:30px;height: 30px;" title="Generar Cita de facturas Seleccionadas" onClick="procedur_stat_fact()" src="images/iconos/citasguardar.png"/>  &nbsp;     </a>
        ...
        <td><div id='msn_procesar_cit'></div></div></table>
         <?php  }
           ?> 
